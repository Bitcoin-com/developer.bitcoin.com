...
1. `randomBits: number` - number of random bits of entropy to use.  More more bits, the longer the mnemonic.
2. `wordList: array optional` - List of words to generate mnemonic from.
...
Create entropy form mnemonic

### Arguments [ ]

1. `mnemonic: string` - a mnemonic to turn to entropy.
2. `mnemonicList: [string] optional` - list of words the mnemonic was generated from
...
`Boolean` true If the mnemonic is valid
...
2. `wordlist: String Array`: wordlist to search
